...
export default function App() {
  ...
  const updateInv = useCallback((pid,field,val)=>{
    setInv(prev=>{
      ...
      ls.set("vkast-inv",next);
      return next;
    });
  },[]);

  const saveCfg = useCallback((nc)=>{ setCfg(nc); ls.set("vkast-cfg",nc); },[]);
  const takeSnap = useCallback((label)=>{
    const snap={id:Date.now(),label,date:new Date().toISOString(),inv:JSON.parse(JSON.stringify(inv))};
    setSnaps(p=>{ const n=[snap,...p].slice(0,12); ls.set("vkast-snap",n); return n; });
  },[inv]);
  ...
  if (loading||!cfg||!inv) return (
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#F0FAE8",fontFamily:"Nunito,sans-serif"}}>
      <style>{GF}</style>
      <div style={{textAlign:"center"}}><div style={{fontSize:52,marginBottom:12}}>🌿</div><div style={{fontSize:14,fontWeight:700,color:"#8AAA7A",letterSpacing:2}}>LADEN...</div></div>
    </div>
  );
        ...
        <div style={{background:"#1A1A2E",border:"2px solid #7C5CBF",borderRadius:24,padding:32,width:"100%",maxWidth:300,textAlign:"center"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
            {[1,2,3,4,5,6,7,8,9].map(n=>(
              <button key={n} style={{height:52,background:"#16213E",border:"2px solid #3D2A7A",borderRadius:12,color:"#C0B0E8",fontFamily:"Nunito,sans-serif",fontSize:20,fontWeight:700,cursor:"pointer"}}
                onClick={()=>{
                  if(adminPin.length>=4)return;
                  const nx=adminPin+n; setAdminPin(nx);
                  if(nx.length===4)setTimeout(()=>{
                    if(nx===(cfg.adminPin||"9999")){setRole("admin");setShowAdmin(false);setAdminPin("");setAdminErr(false);}
                    else{setAdminErr(true);setAdminPin("");}
                  },150);
                }}>{n}</button>
            ))}
            <div/>
            <button style={{height:52,background:"#16213E",border:"2px solid #3D2A7A",borderRadius:12,color:"#C0B0E8",fontFamily:"Nunito,sans-serif",fontSize:20,fontWeight:700,cursor:"pointer"}}
              onClick={()=>{
                if(adminPin.length>=4)return;
          ...
          <div style={{marginTop:14,fontSize:11,color:"#5A4A7A"}}>Standaard PIN: 9999</div>
        </div>
      </div>
      <Ftr isAdmin/>
    </div>
          ...
          <div style={{width:"100%",maxWidth:340,display:"flex",flexDirection:"column",gap:12}}>
            {accounts.map(acc=>(
              <LoginCard key={acc.id} acc={acc}
                onLogin={pw=>pw===acc.password?setRole(acc.role):setLoginErr(acc.id)}
...
function Hdr({cfg,role,isAdmin,onBack,backLabel,onSwitch}){
  return(
    <div style={{width:"100%",background:isAdmin?"#0D0D1A":"#3D8B2E",padding:"11px 14px",display:"flex",alignItems:"center",gap:10,position:"sticky",top:0,zIndex:200,boxShadow:"0 3px 14px rgba(61,139,46,0.25)",overflowX:"auto",scrollbarWidth:"none"}}>
      <span style={{fontSize:20,flexShrink:0}}>{isAdmin?"🔧":"🌿"}</span>
      <div style={{flexShrink:0}}>
        <div style={{fontSize:15,fontWeight:900,color:"#fff",whiteSpace:"nowrap"}}>{isAdmin?"Masterfile":cfg?.appName||"Vloeistoffenkast"}</div>
...
function LoginCard({acc,onLogin,hasErr,onClear}){
  const [pass,setPass]=useState("");
  const [showPw,setShowPw]=useState(false);
  const isMgr=acc.role==="manager";
  return(
    <div style={{background:hasErr?"#FDEDEA":"#fff",border:`2.5px solid ${hasErr?"#D44A2A":isMgr?"#E8632A55":"#C8E6B0"}`,borderRadius:18,padding:18,boxShadow:"0 4px 16px rgba(61,139,46,0.1)",overflow:"hidden"}}>
      ...
      {hasErr&&<div style={{background:"#FDEDEA",border:"1.5px solid #D44A2A",borderRadius:10,padding:"9px 12px",fontSize:12,fontWeight:700,color:"#D44A2A",marginBottom:10}}>Onjuist wachtwoord</div>}
      ...
      <button style={{...S.btn,width:"100%",background:isMgr?"linear-gradient(135deg,#E8632A,#D44A20)":"linear-gradient(135deg,#3D8B2E,#5AAE3C)",color:"#fff",letterSpacing:1,textTransform:"uppercase"}} onClick={()=>onLogin(pass)}>
        {isMgr?"Manager inloggen":"Medewerker inloggen"}
      </button>
    </div>
  );
}
        ...
        <div style={{...S.card,marginTop:12}}>
          ...
          {snaps.map(snap=>(
            <div key={snap.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"7px 0",borderBottom:"1px solid #EEF9E6"}}>
              ...
              <button style={{background:"#FDEDEA",border:"none",color:"#D44A2A",borderRadius:7,width:28,height:28,cursor:"pointer",fontSize:13}} onClick={()=>{const n=snaps.filter(s=>s.id!==snap.id);setSnaps(n);ls.set("vkast-snap",n);}}>×</button>
            </div>
          ))}
        </div>
      )}
...
function AdminPanel({cfg,onSave}){
  ...
  const save=()=>{onSave(local);setSaved(true);setTimeout(()=>setSaved(false),2500);};
  const upd=(path,val)=>{
    setLocal(prev=>{
      ...
      <div style={{width:"100%",maxWidth:460,padding:"14px 14px 0",margin:"0 auto"}}>
        ...
        {tab==="accounts"&&<div>
          {(local.accounts||[]).map((acc,ai_)=><div key={acc.id} style={ac}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
              ...
              <div><label style={al}>Wachtwoord</label><input style={ai} value={acc.password} onChange={e=>upd(`accounts.${ai_}.password`,e.target.value)}/></div>
            </div>
          </div>)}
        ...
        {tab==="instellingen"&&<div>
          <div style={ac}>
            ...
            <div><label style={al}>Beheerder PIN</label><input style={{...ai,width:"50%"}} type="password" value={local.adminPin||""} onChange={e=>upd("adminPin",e.target.value)}/></div>
          </div>
          <button style={sv} onClick={save}>{saved?"Opgeslagen!":"Opslaan"}</button>
        </div>}
...
function ReportModal({cfg,inv,onClose}){
  ...
  if(vp.length>0){
    ...
    vp.forEach(p=>{
      ...
      lines.push(`  ${p.name.padEnd(24)} ${String(cnt).padEnd(8)} doel:${p.target}  ${need>0?`+${need} ${p.unit}s`:"ok"}`);
    });
    lines.push("");
  }
